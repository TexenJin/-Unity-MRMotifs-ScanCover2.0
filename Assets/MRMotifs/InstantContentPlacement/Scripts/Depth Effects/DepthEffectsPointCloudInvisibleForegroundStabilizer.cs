...
namespace MRMotifs.InstantContentPlacement.DepthEffects
{
    ...
    public class DepthEffectsPointCloudInvisibleForegroundStabilizer : MonoBehaviour
    {
        [SerializeField]
        ...
        [SerializeField]
        private Vector3 cameraLocalOffset = new Vector3(0.42f, -0.28f, 0.62f);

        [SerializeField]
        private int textFontSize = 12;

        [SerializeField]
        private float textCharacterSize = 0.00035f;

        [SerializeField]
        private Color textColor = new Color(1f, 1f, 1f, 0.005f);
        ...
        private void EnsureProxy()
        {
            ...
            GameObject textObject = new GameObject("DepthEffectsPointCloudInvisibleForegroundStabilizer");
            ...
            m_textMesh.fontSize = Mathf.Max(8, textFontSize);
            m_textMesh.characterSize = Mathf.Max(0.0001f, textCharacterSize);
        ...
        private void RefreshProxyText()
        {
            ...
            m_builder.Append("voxels=").Append(voxelCount)
                ...
                .Append(" recent=").Append(recentCount).AppendLine();
            m_builder.Append("front=").Append(inFrontCount)
                .Append(" frustum=").Append(inFrustumCount).AppendLine();
            m_builder.Append("renderer=").Append(rendererStableCount)
                .Append('/').Append(rendererDisplayCount)
                .Append('/').Append(rendererBatchCount).AppendLine();
            m_builder.Append("sampler=").Append(sampleAttempts)
                .Append('/').Append(sampleScheduled)
                .Append('/').Append(sampleAccepted)
                .Append('/').Append(sampleOccluded)
                .Append('/').Append(sampleRejectedConfidence);
