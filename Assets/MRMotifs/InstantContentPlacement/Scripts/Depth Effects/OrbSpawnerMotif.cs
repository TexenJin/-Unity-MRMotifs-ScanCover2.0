...
namespace MRMotifs.InstantContentPlacement.DepthEffects
{
    ...
    public class OrbSpawnerMotif : MonoBehaviour
    {
        ...
        [Tooltip("Persistent point cloud map used by shock wave scans.")]
        [SerializeField]
        private DepthEffectsPointCloudMapMotif pointCloudMapMotif;

        [SerializeField]
        private DepthEffectsPointCloudMapMotif.PerformanceSliceMode pointCloudPerformanceSliceMode =
            DepthEffectsPointCloudMapMotif.PerformanceSliceMode.Baseline;
        ...
        private void Awake()
        {
            Debug.LogWarning("[OrbSpawnerMotif] Awake");
            if (Camera.main != null)
            {
                m_cameraTransform = Camera.main.transform;
            }

            if (pointCloudMapMotif == null)
            {
                pointCloudMapMotif = GetComponent<DepthEffectsPointCloudMapMotif>();
            }

            if (pointCloudMapMotif == null)
            {
                pointCloudMapMotif = gameObject.AddComponent<DepthEffectsPointCloudMapMotif>();
                Debug.LogWarning("[OrbSpawnerMotif] Added DepthEffectsPointCloudMapMotif");
            }

            pointCloudMapMotif.SetPerformanceSliceMode(pointCloudPerformanceSliceMode);
            pointCloudMapMotif.Configure(environmentRaycast, Camera.main);
        }
        ...
        private void HandleDepthScan(Vector3 position, ShockWaveOrbMotif shockWaveOrbMotif)
        {
            ...
            var scanEffect = scanEffectAudioSrc.GetComponent<ShockWaveEffectMotif>();
            if (scanEffect != null)
            {
                scanEffect.Initialize(environmentRaycast, Camera.main, pointCloudMapMotif);
            }
